...
include_once "models/BaseObject.php";
 ...
 if( ! class_exists( 'ESIG_License' ) )
  ...
  $license = new ESIG_License( __FILE__, __( 'WP E-Signature', 'esig' ), '1.1.5', __( 'Approve Me', 'esig' ) );
...
final class WP_E_Digital_Signature extends WP_E_BaseObject {
	...
	public function __construct() {
		...
		add_action('template_redirect', array(&$this, 'esign_force_ssl'));
		...
		//remove_filter('template_include','template_context',9999);
	...
	private function settingsEstablished(){
		...
		$setting = new WP_E_Setting();
		// esig doc super admin changed request return false . 
	...
	*/
	...
	public function adminMenu(){
        ...
        if($this->setting->esign_hide_esig_menus()){
		    add_menu_page( __( 'E-Signature', 'esig' ),__( 'E-Signature', 'esig' ), 'read', $this->main_screen, array(&$this, 'route'), ESIGN_ASSETS_DIR_URI . '/images/pen_icon.svg');
		}
		...
		if($this->esigrole->esig_current_user_can('delete_document')){
		...
		if($this->esigrole->esig_current_user_can('have_licenses')){
			...
			add_submenu_page($this->main_screen, __( 'Add-ons', 'esig' ), __( 'Add-ons', 'esig' ), 'read', $prefix.'addons', array(&$this, 'route'));
		}
		...
		//add_submenu_page('esign-docs', 'My Branding', 'My Branding', 'read','email-templates-form', array(&$this, 'route'));
	...
	public function enqueueAdminScripts(){
		...
		if($current_screen = $this->screen_prefix . 'settings'){
...
function wp_e_signature_update_db_check() {
   ...
   $installed_esig_db_ver=get_option( "esig_db_version");
		...
		if(empty($esig_db_version))
					$esig_db_version='1.0.2';
	 
    if($installed_esig_db_ver != $esig_db_version) {
...
{  
	...
	if(preg_match("/esign/", $page))
		{
		$esign_rate_text = sprintf( __( 'Thank you for using <a href="http://www.approveme.me/wp-digital-e-signature/" target="_blank">E-Signature </a>! Please <a href="http://wordpress.org/plugins/wp-digital-e-signature" target="_blank">rate us</a> on <a href="http://wordpress.org/plugins/wp-digital-e-signature" target="_blank">WordPress.org</a>', 'esig' ),
