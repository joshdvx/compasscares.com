...
class ESIG_CUSTOM_MESSAGE_Admin {
	...
	public function document_add_data($more_contents) {
		...
		$more_contents .= sprintf(__('<p id="esig_custom_message_option">
			<a href="#" class="tooltip">
				...
				<input type="checkbox" '. $checked .' id="esig_custom_message" name="esig_custom_message" value="1"> ' . __('Add cutom message to signer invite email','esig-acm') .'
	...
	public function enqueue_admin_scripts() {
		...
		if(in_array($screen->id, $admin_screens)){
			...
			wp_enqueue_script( $this->plugin_slug . '-admin-script', plugins_url( 'assets/js/esig-add-custom-message.js', __FILE__ ), array('jquery', 'jquery-ui-dialog'), ESIG_AT::VERSION ,true);	
		}
   }
