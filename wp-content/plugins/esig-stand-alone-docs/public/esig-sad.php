...
class ESIG_SAD {
	...
	public function display_document($atts){
		...
		// Signing
			...
			$api->notify_owner($doc, $recipient_obj, $audit_hash); // Notify admin
			
			$post = array('invite_hash'=>$invite_hash, 'checksum'=>$doc->document_checksum);
			
			$api->notify_signer($doc, $recipient_obj, $post, $audit_hash); // Notify signer
	...
	public function enqueue_styles() {
		...
		// If we're on a stand alone page
		if( is_page($current_page) && in_array($current_page, $this->sad_pages)){	
		 echo "<link rel='stylesheet' id='esig-sad-plugin-styles-css'  href='". plugins_url() ."/esig-stand-alone-docs/public/assets/css/public.css?ver=". self::VERSION ."' type='text/css' media='all' />";	
		}
	}
	...
	public function enqueue_scripts() {
		...
		// If we're on a stand alone page
		if( is_page($current_page) && in_array($current_page, $this->sad_pages)){
		echo "<script type='text/javascript' src='". plugins_url() ."/esig-stand-alone-docs/public/assets/js/public.js?ver=". self::VERSION ."'></script>";
		}
	}
