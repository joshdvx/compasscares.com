...
class WP_E_aboutsController extends WP_E_appController {
	...
	private function queueScripts(){
		...
		wp_enqueue_script('esig-common-js', ESIGN_ASSETS_DIR_URI  . "/js/common.js");
	}
	...
	public function systeminfo($data_return=false)
	{
	  ...
	  $memory = WP_MEMORY_LIMIT;
				...
				if ( $memory < 67108864 ) {
					$memorytxt= '<mark class="error">' . sprintf( __( '%s - We recommend setting memory to at least 64MB. See: <a href="%s">Increasing memory allocated to PHP</a>', 'esig' ), size_format( $memory ), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</mark>';
				} else {
					$memorytxt = '<mark class="yes">' . size_format( $memory ) . '</mark>';
				}
