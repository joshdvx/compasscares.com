...
class WP_E_DocumentsController extends WP_E_appController {
	...
	public function index(){
		...
		foreach($docs as $doc){
			...
			$preview_url = add_query_arg(array('esigpreview'=>1, 'document_id'=>$doc->document_id), get_permalink($pageID));
			...
			if($doc->document_status == 'trash') {
				$row_actions .= sprintf( __( '<span class="restore"><a href="?page=esign-restore-document&document_id=%d">Restore</a></span>', 'esig' ), $doc->document_id );
	...
	public function add(){
		...
		if(count($_POST) < 1){
			...
			$print_option_1= $this->settings->get("esig_print_option")==1 ? "selected" : '' ;
			$print_option_2= $this->settings->get("esig_print_option")==2 ? "selected" : '' ;
			$print_option_3=$this->settings->get("esig_print_option")==3 ? "selected" : '' ; 
			$print_option_4= $this->settings->get("esig_print_option")==4 ? "selected" : '' ;
			if($print_option_1==null && $print_option_2==null && $print_option_3==null && $print_option_4==null){
					$print_option_3="selected";
					}
			} else {
			${"print_option_".$document_print_filter}="selected";
			}
		...
		else 
		{
			...
			if($doc->add_signature){
				try{
                    ...
                    $owner = $this->user->getUserBy('wp_user_id', $doc->user_id);
                    
					$signature = $this->signature->getSignatureData($owner->user_id);
	...
	**/
	public function check_license_validity() {
			...
			if($this->license_active=="valid")
			{
                    ...
                    {
						$license_active=$this->general->license_checking($this->license_key,$this->item_plugshortname) ; 
