...
class WP_E_Common extends WP_E_Model {
	...
	public function esig_document_search_form()
	{
		...
		$html .='<input type="text" name="esig_document_search" style="min-width:250px;" placeholder="Document title or Signer name" value="">
	...
	public function esig_save_administrator(){
		...
		if($wpipd == $admin_user_id || $admin_user_id==null){
			...
			foreach ($blogusers as $buser) {
				...
				$html .='<option value="'. $buser->ID .'" data-used="'. $wpipd . '" '. $selected .'>'. $buser->display_name .' </option>';	
			}
			$html .="</select>";
			return $html ; 
		}else {
    ...
    */
     public function esig_report_bug_form(){
            ...
            // report bug form part 1
               ...
               $report_bug_html .= ' it looks You do not have WP e-Signature valid license .  Please purchase a valid license first. <a href="http://www.esign.approveme.me/buy-wordpress-electronic-signature/" target="_blank">Buy Now</a></p></div>';
  ...
  <input type="hidden" name="fullname" placeholder="'. $first_name .'"  value="'. $first_name .'">
            ...
            <textarea name="field[14]" rows="5" style="display:none;clear:both" cols="40" placeholder="System Info"> '.  htmlspecialchars($system_result)  .' </textarea>
            ...
            <textarea name="field[19]" rows="5" cols="40"></textarea><br>
			 ...
			 <label>
                <input type="checkbox" name="field[20][]" value="I understand this is NOT a support ticket but rather a bug request therefore it will not be handled like a support ticket and i will not receive a response." >
                I understand this is NOT a support ticket but rather a bug request therefore it will not be handled like a support ticket and i will not receive a response.
              </label>
