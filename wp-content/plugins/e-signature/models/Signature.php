...
class WP_E_Signature extends WP_E_Model {
	...
	public function add($signatureJSON, $user_id){
		...
		$salt = hash('sha1', mcrypt_create_iv(32)); // 40 chars
		...
		$this->wpdb->query(
			$this->wpdb->prepare(
				"INSERT INTO $this->table (user_id, signature_hash, signature_salt, signature_data, signature_added) 
				 VALUES(%d,'%s','%s','%s','%s')", $user_id, $signature_hash, $salt, $encrypted_signature, $date
			)
		);
