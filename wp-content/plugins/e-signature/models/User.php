...
class WP_E_User extends WP_E_Model{
	...
	public function getUserdetails($user_id, $document_id){
		...
		if($this->settings->get_generic("esign_user_meta_id_". $user_id ."_name_document_id_".$document_id)){
		 ...
		 $user[0]->first_name = $this->settings->get_generic("esign_user_meta_id_". $user_id ."_name_document_id_".$document_id) ; 
				...
				$user[0]->first_name = $user[0]->first_name . " " .$user[0]->last_name;
			}
			return $user[0];
		}
		else { return false; }
	...
	public function checkEsigAdmin($user_id){
		...
		if(defined(ESIGN_DOC_SUPERADMIN_USERID) && ESIGN_DOC_SUPERADMIN_USERID===true){
	...
	public function esig_get_administrator_displayname(){
		...
		return  $user_details->display_name ;
